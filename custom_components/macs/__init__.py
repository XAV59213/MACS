...
def _migrate_entity_id(hass: HomeAssistant, unique_id: str, desired_entity_id: str) -> None:
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Migration propre des entity_id.
    _migrate_entity_id(hass, "macs_mood", "select.macs_mood")
    _migrate_entity_id(hass, "macs_debug", "select.macs_debug")

    _migrate_entity_id(hass, "macs_brightness", "number.macs_brightness")
    _migrate_entity_id(hass, "macs_battery_charge", "number.macs_battery_charge")
    _migrate_entity_id(hass, "macs_temperature", "number.macs_temperature")
    _migrate_entity_id(hass, "macs_windspeed", "number.macs_windspeed")
    _migrate_entity_id(hass, "macs_precipitation", "number.macs_precipitation")

    # Correction importante : cette ligne manquait.
    _migrate_entity_id(hass, "macs_animations_enabled", "switch.macs_animations_enabled")

    _migrate_entity_id(hass, "macs_charging", "switch.macs_charging")
    _migrate_entity_id(hass, "macs_weather_conditions_snowy", "switch.macs_weather_conditions_snowy")
    _migrate_entity_id(hass, "macs_weather_conditions_cloudy", "switch.macs_weather_conditions_cloudy")
    _migrate_entity_id(hass, "macs_weather_conditions_rainy", "switch.macs_weather_conditions_rainy")
    _migrate_entity_id(hass, "macs_weather_conditions_windy", "switch.macs_weather_conditions_windy")
    _migrate_entity_id(hass, "macs_weather_conditions_sunny", "switch.macs_weather_conditions_sunny")
    _migrate_entity_id(hass, "macs_weather_conditions_stormy", "switch.macs_weather_conditions_stormy")
    _migrate_entity_id(hass, "macs_weather_conditions_foggy", "switch.macs_weather_conditions_foggy")
    _migrate_entity_id(hass, "macs_weather_conditions_hail", "switch.macs_weather_conditions_hail")
    _migrate_entity_id(hass, "macs_weather_conditions_lightning", "switch.macs_weather_conditions_lightning")
    _migrate_entity_id(hass, "macs_weather_conditions_partlycloudy", "switch.macs_weather_conditions_partlycloudy")
    _migrate_entity_id(hass, "macs_weather_conditions_pouring", "switch.macs_weather_conditions_pouring")
    _migrate_entity_id(hass, "macs_weather_conditions_clear_night", "switch.macs_weather_conditions_clear_night")
    _migrate_entity_id(hass, "macs_weather_conditions_exceptional", "switch.macs_weather_conditions_exceptional")

    # Suppression de l'ancien debug switch si présent.
    registry = er.async_get(hass)
    legacy_debug = next(
        (
            entity
            ...
            and entity.unique_id == "macs_debug"
    ...
    # Cache les entités MACS d'Assist par défaut.
    ...
    async def handle_set_mood(call: ServiceCall) -> None:
        ...
        entity_id = _find_entity_id_by_unique_id(hass, "macs_mood")
    ...
    async def _set_number_entity(
        ...
        entity_id = _find_entity_id_by_unique_id(hass, unique_id)

        if not entity_id:
            raise vol.Invalid(f"Macs {label} entity not found. Entity unique_id={unique_id} is missing.")
    ...
    service_definitions = [
        (
            SERVICE_SET_MOOD,
            handle_set_mood,
            vol.Schema({vol.Required(ATTR_MOOD): vol.In(MOODS)}),
        ),
        (
            SERVICE_SET_BRIGHTNESS,
            handle_set_brightness,
            vol.Schema({vol.Required(ATTR_BRIGHTNESS): vol.Coerce(float)}),
        ),
        (
            SERVICE_SET_TEMPERATURE,
            handle_set_temperature,
            vol.Schema({vol.Required(ATTR_TEMPERATURE): vol.Coerce(float)}),
        ),
        (
            SERVICE_SET_WINDSPEED,
            handle_set_windspeed,
            vol.Schema({vol.Required(ATTR_WINDSPEED): vol.Coerce(float)}),
        ),
        (
            SERVICE_SET_PRECIPITATION,
            handle_set_precipitation,
            vol.Schema({vol.Required(ATTR_PRECIPITATION): vol.Coerce(float)}),
        ),
        (
            SERVICE_SET_BATTERY_CHARGE,
            handle_set_battery_charge,
            vol.Schema({vol.Required(ATTR_BATTERY_CHARGE): vol.Coerce(float)}),
        ),
        (
            SERVICE_SET_ANIMATIONS_ENABLED,
            handle_set_animations_enabled,
            vol.Schema({vol.Required(ATTR_ANIMATIONS_ENABLED): cv.boolean}),
        ),
        (
            SERVICE_SET_CHARGING,
            handle_set_charging,
            vol.Schema({vol.Required(ATTR_CHARGING): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_SNOWY,
            handle_set_weather_conditions_snowy,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_SNOWY): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_CLOUDY,
            handle_set_weather_conditions_cloudy,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_CLOUDY): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_RAINY,
            handle_set_weather_conditions_rainy,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_RAINY): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_WINDY,
            handle_set_weather_conditions_windy,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_WINDY): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_SUNNY,
            handle_set_weather_conditions_sunny,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_SUNNY): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_STORMY,
            handle_set_weather_conditions_stormy,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_STORMY): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_FOGGY,
            handle_set_weather_conditions_foggy,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_FOGGY): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_HAIL,
            handle_set_weather_conditions_hail,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_HAIL): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_LIGHTNING,
            handle_set_weather_conditions_lightning,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_LIGHTNING): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_PARTLYCLOUDY,
            handle_set_weather_conditions_partlycloudy,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_PARTLYCLOUDY): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_POURING,
            handle_set_weather_conditions_pouring,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_POURING): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_CLEAR_NIGHT,
            handle_set_weather_conditions_clear_night,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_CLEAR_NIGHT): cv.boolean}),
        ),
        (
            SERVICE_SET_WEATHER_CONDITIONS_EXCEPTIONAL,
            handle_set_weather_conditions_exceptional,
            vol.Schema({vol.Required(ATTR_WEATHER_CONDITIONS_EXCEPTIONAL): cv.boolean}),
        ),
        (
            SERVICE_SEND_USER_MESSAGE,
            handle_send_user_message,
            vol.Schema({vol.Required(ATTR_MESSAGE): cv.string}),
        ),
        (
            SERVICE_SEND_ASSISTANT_MESSAGE,
