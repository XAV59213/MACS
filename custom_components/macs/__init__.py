...
async def _ensure_lovelace_resource(hass: HomeAssistant) -> None:
    ...
    In YAML mode, HA does not expose a writable resources collection, so we no-op.
    """
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # Entity ID migration.
    # Fix ugly auto-generated IDs like select.m_a_c_s_weather.
    ...
    def migrate(unique_id: str, desired_entity_id: str) -> None:
        entry_obj = next(
            (
                ...
                if entity_entry.platform == DOMAIN and entity_entry.unique_id == unique_id
            ),
            None,
        )
    ...
    migrate("macs_animations_enabled", "switch.macs_animations_enabled")
    ...
    legacy_debug = next(
        (
            ...
            and entity_entry.unique_id == "macs_debug"
    ...
    # Hide MACS entities from Assist by default.
    if not entry.options.get("assist_exposure_initialized"):
        for entity_entry in list(reg.entities.values()):
            if entity_entry.platform != DOMAIN or entity_entry.config_entry_id != entry.entry_id:
                continue

            options = dict(entity_entry.options)
            ...
            try:
                reg.async_update_entity_options(entity_entry.entity_id, DOMAIN, options)
            except AttributeError:
                try:
                    reg.async_update_entity(entity_entry.entity_id, options=options)
                except TypeError:
                    pass
        ...
        hass.config_entries.async_update_entry(
            ...
            options={
                **entry.options,
                "assist_exposure_initialized": True,
            },
        )
    ...
    async def handle_set_mood(call: ServiceCall) -> None:
        ...
        for entity_entry in registry.entities.values():
            if entity_entry.platform == DOMAIN and entity_entry.unique_id == "macs_mood":
                entity_id = entity_entry.entity_id
                break

        if not entity_id:
            raise vol.Invalid("Macs mood entity not found.")
    ...
    async def _set_number_entity(
        ...
        if not entity_id:
            raise vol.Invalid(f"Macs {label} entity not found.")
    ...
    for service_name, handler, attr_name in weather_services:
        if not hass.services.has_service(DOMAIN, service_name):
            hass.services.async_register(
                DOMAIN,
                service_name,
                handler,
                schema=vol.Schema({vol.Required(attr_name): cv.boolean}),
            )
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if unload_ok and not hass.config_entries.async_entries(DOMAIN):
        ...
        hass.services.async_remove(DOMAIN, SERVICE_SET_ANIMATIONS_ENABLED)
